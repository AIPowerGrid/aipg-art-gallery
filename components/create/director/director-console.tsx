...
export function DirectorConsole({ styles, walletAddress, authenticated }: DirectorConsoleProps) {
  const router = useRouter();
  const { openConnectModal } = useConnectModal();
  const { address, isConnected } = useAccount();
  const { signMessageAsync } = useSignMessage();

  // Working connect flow: open the RainbowKit modal (providers auto-run SIWE
  // after connect); if the wallet is connected but the session isn't signed,
  // re-trigger the SIWE signature directly.
  const handleConnect = useCallback(async () => {
    ...
    if (address) {
      try {
        ...
        console.warn("[Director] sign-in failed", err);
      }
    }
  }, [isConnected, address, openConnectModal, signMessageAsync, router]);
  ...
  const modelReady = !!model?.enabled;
