...
1. **Supabase Setup** (Required for gallery and authentication):
   ```bash
   # Run setup script to open Supabase dashboard pages
   node scripts/open-setup.js
   ```
   Follow the instructions to:
   - Execute SQL schema from `supabase/schema.sql`
   - Configure OAuth providers (GitHub, Google, Facebook, Twitter, Apple)
...
The API server runs on port 4000, frontend on port 3000.
...
- **Public Gallery**: Browse all publicly shared images and videos generated by the community
- **User Authentication**: OAuth login with GitHub, Google, Facebook, X (Twitter), and Apple
- **User Profiles**: Save and manage your own creations with public/private visibility controls
- **All 19 Models Supported**: Full support for all workflow models from comfy-bridge
...
- Automatic saving of generations to database when shared publicly
